...
class ViewController: NSViewController {
    ...
    var activeItems = [FileMonitoringEvent]()
    // event array of all events
    var savedItems = [FileMonitoringEvent]()
    ...
    override func viewWillAppear() {
        //Add feature to move app to application folder
    }
    ...
    func logEvent(event: String) {
        ...
        guard let jsonData = event.data(using: .utf8),
            let FileMonitoringEvent = try? decoder.decode(FileMonitoringEvent.self, from: jsonData)
        else {
            NSLog("Failed to deseralize json into fileMonitoring event")
            return
        }
        ...
        DispatchQueue.main.async {
            ...
            self.savedItems.append(FileMonitoringEvent)
            self.addEventIfFilterIsSet(event: FileMonitoringEvent)
            self.reloadEvents(force: false)
            //save to log file
            self.logEventsToFile()
        }
    }
    ...
    func addEventIfFilterIsSet(event: FileMonitoringEvent) {
...
extension ViewController: NSTableViewDelegate {
    ...
    //Display all file related events for fill access monitoring
    func displayEventDetails(event: FileMonitoringEvent) {
        switch event.eventtype {
        case "file::open":
            eventLabel.stringValue = "File open Event:📂 \(event.props["path"] ?? "<missing>")"
        case "file::close":
            eventLabel.stringValue = "File close Event:📂 \(event.props["path"] ?? "<missing>")"
        case "file::create":
            eventLabel.stringValue = "File create Event:📂 \(event.props["path"] ?? "<missing>")"
        case "file::clone":
            eventLabel.stringValue = "File clone Event:📂 \(event.props["path"] ?? "<missing>")"
        case "file::update":
            eventLabel.stringValue = "File update Event:📂 \(event.props["path"] ?? "<missing>")"
        case "file::rename":
            eventLabel.stringValue = "File rename Event:📂 \(event.props["path"] ?? "<missing>")"
        case "file::unlink":
